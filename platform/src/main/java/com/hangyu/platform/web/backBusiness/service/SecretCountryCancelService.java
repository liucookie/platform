 ...
 */
@Service
public class SecretCountryCancelService {

    @Resource
    private NationlevelremoveTblMapper mapper;
     ...
     * @return
     */
    public NationlevelremoveTbl queryById(String id) {
        NationlevelremoveTbl tbl = mapper.selectByPrimaryKey(id);
        return tbl;
    }
     ...
     */
    @Transactional
    public void add(NationlevelremoveTbl tbl, MultipartFile file) throws IOException {
        ...
        tbl.setFormid(X5CommonFields.getSecretCountryCancelApproveId() + timeStamp);

        // 调用宏天工作流--启动流程（返回流程实例id）
        String instanceId = eipUtils.startTask(X5CommonFields.getSecretCountryCancelApproveId(),tbl.getAccoutnamer());
