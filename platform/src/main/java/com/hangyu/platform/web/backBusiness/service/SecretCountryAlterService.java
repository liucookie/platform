 ...
 */
@Service
public class SecretCountryAlterService {

    @Resource
    private NationlevelalterTblMapper mapper;
     ...
     * @return
     */
    public NationlevelalterTbl queryById(String id) {
        NationlevelalterTbl tbl = mapper.selectByPrimaryKey(id);
        return tbl;
    }
     ...
     */
    @Transactional
    public void add(NationlevelalterTbl tbl, MultipartFile file) throws IOException {
        ...
        tbl.setFormid(X5CommonFields.getSecretCountryAlterApproveId() + timeStamp);

        // 调用宏天工作流--启动流程（返回流程实例id）
        String instanceId = eipUtils.startTask(X5CommonFields.getSecretCountryAlterApproveId(),tbl.getAccoutnamer());
