 ...
 */
@Service
public class MediaManageApproveService {

    @Resource
    private MediamanagementapprovalMapper mapper;
     ...
     * @return
     */
    public Mediamanagementapproval queryById(String id) {
        Mediamanagementapproval tbl = mapper.selectByPrimaryKey(id);
        return tbl;
    }
     ...
     */
    @Transactional
    public void add(Mediamanagementapproval tbl) throws IOException {
        ...
        tbl.setFormid(X5CommonFields.getMediaManageApproveId() + timeStamp);

        // 调用宏天工作流--启动流程（返回流程实例id）
        String instanceId = eipUtils.startTask(X5CommonFields.getMediaManageApproveId(), tbl.getAccoutnamer());
