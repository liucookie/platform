 ...
 */
@Service
public class NationFileAlterService {

    @Resource
    private NationlevelfilealterTblMapper mapper;
     ...
     * @return
     */
    public NationlevelfilealterTbl queryById(String id) {
        NationlevelfilealterTbl tbl = mapper.selectByPrimaryKey(id);
        return tbl;
    }
     ...
     */
    @Transactional
    public void add(NationlevelfilealterTbl tbl, MultipartFile file) throws IOException {
        ...
        tbl.setFormid(X5CommonFields.getNationFileAlterApproveId() + timeStamp);

        // 调用宏天工作流--启动流程（返回流程实例id）
        String instanceId = eipUtils.startTask(X5CommonFields.getNationFileAlterApproveId(),tbl.getAccoutnamer());
